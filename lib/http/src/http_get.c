...
int http_get(int sock, request *req) {
  if (!req) {
    L_ERR_THR("http", "get", "Request not provided");
    return HTTP_ERROR;
  }
  ...
  if (http_create_full_path(full_path, req->uri) != HTTP_SUCCESS) {
    L_ERR_THR("http", "get", "Can't create full path to file");
    return HTTP_ERROR;
  }
  ...
  if (is_dir) {
    if (http_create_index_path(full_path) != HTTP_SUCCESS) {
      L_ERR_THR("http", "get", "Can't create path to index file of directory");
      return HTTP_ERROR;
    }
  }
  L_DEB_THR("http", "get", "File path is %s", full_path);
  ...
  if (!file) {
    switch (errno) {
      default: {
        L_ERR_THR("http", "get", strerror(errno));
        return HTTP_ERROR;
      }
  ...
  if (http_content_type(&content_type, full_path) != HTTP_SUCCESS) {
    L_ERR_THR("http", "get", "Can't get content-type of file");
    fclose(file);
    return HTTP_ERROR;
  }
  ...
  if (!bytes) {
    L_DEB_THR("http", "get", "Can't create headers: %s", strerror(errno));
    fclose(file);
    return HTTP_ERROR;
  }
  ...
  if (send(sock, responce, bytes, 0) < bytes) {
    L_DEB_THR("http", "get", "Can't send headers: %s", strerror(errno));
    fclose(file);
    return HTTP_ERROR;
  }
